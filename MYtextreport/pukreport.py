...
#from gramps.gen.plug.report import find_marriage
...
from gramps.gen.utils.db import get_primary_event_ref_list
        ...
        for pe in self.__db.get_person_handles():
            for eventref in self.__db.get_person_from_handle(pe).event_ref_list:
                if not eventref.get_role().is_primary():
                    ...
                    elif eventref.get_role() == "Trauzeuge":
                        tzdic[eventref.ref].append((eventref.get_role(),pe))
                        indexlist.append(self.__db.get_person_from_handle(pe).gramps_id)
        gpdickeys = gpdic.keys()
        tzdickeys = tzdic.keys()
...
##        column_titles = [_("Prim_ID"), _("Gramps_ID"),  _("Role"), _("Relation"), _("LNr"), _("Ev ID"),_("Event"), _("Date"), _("Year"), _("Age Primary"), _("Age Person"), _("gender Primary"), _("gender Person") , _("Person"),_("Primary"), _("Person birth"), _("Person Clan"),_("Primary Clan"),_("Event Place"),_("FamID"),_("AnzKind")] 
        column_titles = [_("Id"), "Status", "FatherId", "MotherId", "Children", "HUSB_ORD", "WIFE_ORD", "MARR_DAT", "FAMI_RES",_("Fam_ID"), _("Father_ID"), _("Mother_ID"), _("Children_ID")] 
        ...
        for f in sorted(famlist):
            ...
            famidtxt = (" %s" % toNbr(family.gramps_id))
            famreltxt = family.get_relationship()
            marriagedatetxt = family
...
#                print("FAM    ",famreltxt)
            ...
            if marriage:
                marriagedatetxt = self.__format_date(marriage.get_date_object())
            residencetxt = self.find_residence(family)
...
#            line.append(fid)
            ...
            line.append(marriagedatetxt) 
            line.append(residencetxt[4:-2]) 
...
#            print(m)
        ...
        for il in indexlist:
            ...
            for eventref in person.get_event_ref_list():
                ...
                if eventref.get_role().is_primary() and event.type.is_baptism():
...
# Date
        ...
        self.progress.set_pass(_('Start Table Trauzeugen'), len(indexlist))

        self.doc.start_table("Trauzeugen", "SRC-VISONETable")
        column_titles = [_("Id"), "Name", "Family", "Trauzeuge", "#DATE"]
#_("Id"), "Name", "Candidate", "Godparent", "#DATE"
#
#
        self.doc.start_row()
        self.doc.start_cell("SRC-TableColumn")
        self.doc.start_paragraph("SRC-ColumnTitle")
        self.doc.write_text("Trauzeugen")
        self.doc.end_paragraph()
        self.doc.end_cell()
        self.doc.end_row()
#
...
#            print(m)
        pi = 1
        for f in sorted(famlist):
            fam = self.__db.get_family_from_handle(f)
            if fam:
                print("FFFFFFFFF    ",len(get_primary_event_ref_list(self.__db, fam)))
                for event_ref in get_primary_event_ref_list(self.__db, fam):
                    if event_ref:
                        event = self.__db.get_event_from_handle(event_ref.ref)
...
# Trauzeugen
                            ...
                            tztxt =""
                            for i in tzdickeys:
     #                           print(self.__db.get_event_from_handle(i).gramps_id, event.gramps_id)
                                if self.__db.get_event_from_handle(i).gramps_id == event.gramps_id:
                                    print(event.gramps_id)
                                    for (a,b) in tzdic[i]:
                                        tztxt = tztxt + toNbr(self.__db.get_person_from_handle(b).gramps_id) +";"
