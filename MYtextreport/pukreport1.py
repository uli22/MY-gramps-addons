...
#            for (a,b) in gpdic[i]:
#                print("TEIL     ",a
#                        ,name_displayer.display(self.database.get_person_from_handle(b))
#                        ,self.database.get_person_from_handle(b).gramps_id)
        print("GPDIC   ",len(gpdic.keys()))
...
#                birthplacetxt = self.find_place(self.__db.get_event_from_handle(birth_ref.ref).place)
                birthplacetxt = place_displayer.display_event(self.database, self.__db.get_event_from_handle(birth_ref.ref), self.pl_format)
...
# Death Date
            ...
            if death_ref:
                ...
                deathplacetxt = place_displayer.display_event(self.database, self.__db.get_event_from_handle(death_ref.ref), self.pl_format)
            self.doc.write_text(_("%s") %
                                deathtxt)
            self.doc.end_paragraph()
            self.doc.end_cell()
...
# Godparents
                    ...
                    for i in gpdic.keys():
                        if self.__db.get_event_from_handle(i).gramps_id == event.gramps_id:
                            for (a,b) in gpdic[i]:
                                patentxt = patentxt + toNbr(self.database.get_person_from_handle(b).gramps_id) +";"

                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
                    self.doc.write_text(_("PATEN %s") %
                                        patentxt)
                    self.doc.end_paragraph()
                    self.doc.end_cell()
