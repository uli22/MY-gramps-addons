...
#------------------------------------------------------------------------
#
# PeopleCitationsEventRoleRelationXLSReportPath
#
#------------------------------------------------------------------------
class PeopleCitationsEventRoleRelationXLSReportPath(Report):
    """
...
#
###########################################
 #           (relation, Ga, Gb, Pa, Pb) = rel_calc.get_one_relationship(
...
#            #(rank, person_handle, firstRel, firstFam, secondRel, secondFam),msg = rel_calc.get_relationship_distance_new( self.__db, 
#            data, msg = rel_calc.get_relationship_distance_new( self.__db, 
#                                                    self.__db.get_person_from_handle(primpershandle),
#                                                    self.__db.get_person_from_handle(rolepershandle),
#                                                    all_families=True, 
#                                                    all_dist=True, 
...
##UU#############################
    ...
    def personrelationpath1(self, rolepershandle, primpershandle):
        #*************************
        # Hier MFZDS    AKTUELL IN BEARBEITUNG

        stop = False

        if not stop and primpershandle == rolepershandle:
            rel_str = ''
            stop = True
...
#          #most relevant relationship is a birth family relation of lowest rank
        ...
        for rel in data :
            ...
            if rel[0] == rankbest:
                databest.append(rel)
            relation_text = relation_text + "(%d %s %s %s %s %s )" % rel
            
#        data = self.collapse_relations(data)
        relation_text = relation_text + "(AFTER   %d %s %s %s %s %s )" % databest[0]
#        relation_text = relation_text + "(AFTER   %d %s %s %s %s )" % databest[0][0], databest[0][2], databest[0][3], databest[0][4], databest[0][5]                                                   
#        relation_text = "(AFTER   %d %s %s %s %s %s)" % data[0][0], data[0][1], data[0][2], data[0][3], data[0][4], data[0][5]                                                   
#        relation_text = "(AFTER   %d)" % databest[0][2]                                                   
#       
#            relation_text = "(%d %s %s %s %s %s)" % data
        return (_("%s") % relation_text)
...
#                        self.personrelation(Fam, e, name_handle, primarypersonAhandle, primarypersonBhandle, 1))
                    ...
                    self.doc.start_cell("SRC-Cell")
                    self.doc.start_paragraph("SRC-SourceDetails")
#U                    self.doc.write_text(_("%s") %
#U                        self.personrelationpath(e, name_handle))
                    PRPathTuple =self.personrelationpath(e, name_handle)
                    PRPath = PRPathTuple[0]+" " +str(PRPathTuple[1])+" " +str(PRPathTuple[2])
                    self.doc.write_text(PRPath)
#U                    self.doc.write_text(self.personrelationpath(e, name_handle))
...
#
#------------------------------------------------------------------------
class PeopleCitationsEventRoleRelationXLSOptionsPath(MenuReportOptions):
    """
...
#        para.set(first_indent=-1.5, lmargin=1.5)
    ...
    def __event_table_style(self):
        ...
        table.set_column_width(15, 5)
        table.set_column_width(16, 5)
        table.set_column_width(17, 5)
        table.set_column_width(18, 5)
        table.set_column_width(19, 5)
        table.set_column_width(20, 5)
