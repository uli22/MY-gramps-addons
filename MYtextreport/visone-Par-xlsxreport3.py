            ...
            if Fam == 0:
                ...
                if relation:                                      
                    ...
                    if relation_text[0]=="E":
                        ...
                        relation_text = ("Ehepartner",kp)
#                    is_sp = self.is_spouse(self.__db,
#                                        self.__db.get_person_from_handle(primpershandle),
#                                        self.__db.get_person_from_handle(rolepershandle),
#                                        all_rel = False)
#                    if is_sp:
#                        relation_text = (_("spouse "),"SPOUSE")
############################
                elif self.__db.get_person_from_handle(primpershandle).gramps_id == self.__db.get_person_from_handle(rolepershandle).gramps_id: 
                    relation_text = (_("IP "),"")
                elif relation_text[0] == "nicht verwandt":
#    spouse
...
#                                    "m")
    ...
    #                                    (d)) 
                ...
                #Place of second Person
                #
                # if Second person has residence event returns Place of that else returns birth-place
                #
                ResPlace ="RESI"
                if birth_ref:
                    ResPlace = self.find_place(self.__db.get_event_from_handle(birth_ref.ref))
                self.doc.start_cell("SRC-Cell")
                self.doc.start_paragraph("SRC-SourceDetails")
                self.doc.write_text(_("%s") % ResPlace)
                self.doc.end_paragraph()
                self.doc.end_cell()
...
#UUUUUUU
        ...
        if fam_handle:
            ...
            FID=self.__db.get_family_from_handle(fam_handle).gramps_id
            marriage = get_marriage_or_fallback(self.__db, family)
            AnzKind= len(self.__db.get_family_from_handle(fam_handle).child_ref_list)
...
#                                    name_id)
    ...
    #                   relation_text = "nicht verwandt"
                ...
                if marriage:
                    self.doc.write_text(_("%s") %
                                marriage.get_date_object().get_year())
                else:
                    self.doc.write_text(_("%s") %
                                "")
                self.doc.end_paragraph()  
...
#4.1        evkeys.sort
        ...
        column_titles = [_("Prim_ID"), _("Gramps_ID"),  _("Role"), _("Relation"), _("Kinship Path prim ->pers"), _("LNr"), _("Ev ID"),_("Event"), _("Date"), _("Year"), _("Age Primary"), _("Age Person"), _("gender Primary"), _("gender Person"),_("Primary") , _("Person"), _("Person birth"),_("Person residence"), _("Person Clan"),_("Primary Clan"),_("Event Place"),_("FamID"),_("AnzKind")] 
        i = 0
        self.doc.start_row()
