...
#########    
      #sorted(string_list, key=glocale.sort_key)    
            #print(sorted(fam_list, key=lambda x: (x[0], x[7])))
            #print(sorted(fam_list, key=glocale.sort_key))

    #            logging.info(index)
    #        for ifn in fam_index.keys():
    #            logging.info(ifn, fam_index[ifn])
...
##                    surname = _("unknown")
            ...
            #keys = sorted(fam_list, key=lambda x: glocale.sort_key(fam_list[1]))
            for fn in sorted(fam_list,  key=lambda t: (locale.strxfrm(t[1]), t[7])):
                print(fn)
#            stop
                        ...
                        if self.showgodparents:
                            if len(Patenlist)>0:
                                ...
                                for i,(pa_a,pa_b) in enumerate(Patenlist):
                                    ...
                                    pate_name = self.db.get_person_from_handle(pa_b).get_primary_name().get_first_name() +" "+ self.db.get_person_from_handle(pa_b).get_primary_name().get_surname() 
