...
class MYPlaceReport(Report):
    ...
    def __get_citation(self, cit_handle):
        """
        This procedure gets citation and formats like "page source.abrev [id]"
		input citation_handle
		output citation_string page source.abrev [id]
        """
        cit = self.database.get_citation_from_handle(cit_handle)
        sour = self.database.get_source_from_handle(cit.source_handle)
        citation = sour.abbrev
        citation = citation + ':' + str(cit.page) + ' [' + str(cit.gramps_id)+'],'
        return (citation)

    def __get_source(self, cit_handle):
        """
        This procedure gets citation and formats like "page source.abrev [id]"
		input citation_handle
		output citation_string page source.abrev [id]
        ...
        sour = self.database.get_source_from_handle(cit.source_handle)
        #source = sour.abbrev
        #citation = source.abbrev
        #citation = citation + ':' + str(cit.page) + ' [' + str(cit.gramps_id)+'],'
        #source = source + ':   ' + sour.title + ' [' + str(sour.gramps_id)+'],'
        return ((sour.abbrev, sour.title, ' [' + str(sour.gramps_id)+']'))

    def __get_event_role(self, person, eventhandle):
        """
        return the role a person has in given event
        """
#        rolestr="Hallo"
        for evt_ref in person.event_ref_list:
            rolestr=""
            if evt_ref.ref == eventhandle:
                if (evt_ref.get_role().is_primary()) or (evt_ref.get_role() is None):
#                    print(self._nd.display(person), evt_ref.get_role())
                    rolestr="1"
                else:
                    rolestr="2als " + str(evt_ref.get_role())
#                print(self.database.get_event_from_handle(eventhandle).get_type(), evt_ref.get_role())
                return(rolestr)
        return("")
    ...
    def __write_referenced_events(self, handle):
        ...
        for evt_handle in event_handles:
            ...
            if event: # will be None if marked private
                ...
                citation_list = [self.__get_citation(e) for e in event.citation_list]
                source_list.extend([self.__get_source(e) for e in event.citation_list])
...
#######
#                person_role_list = sorted([(__get_event_role(person, evt_handle), person) for person in person_list]), key=lambda x: x[0])
                person_role_list = sorted([(self.__get_event_role(self.database.get_person_from_handle(p_handle), evt_handle), self.database.get_person_from_handle(p_handle)) for p_handle in person_list], key=lambda x: x[0])
                print(len(person_role_list))
                for r, p in person_role_list:
                    print(r, self._nd.display(p), p.get_gramps_id)
                for r, person in person_role_list:
#######
##                for p_handle in person_list:
#                    person = self.database.get_person_from_handle(p_handle)
                    if person:
                        if people == "":
                            people = "%(name)s (%(id)s) %(role)s" \
                                     % {'name': self._nd.display(person),
                                        'id': person.get_gramps_id(),
										'role': self.__get_event_role(person, evt_handle)[1:]}
                        else:
                            people = self._("%(persons)s und %(name)s "
                                            "(%(id)s) %(role)s") \
                                     % {'persons': people,
                                        'name': self._nd.display(person),
                                        'id': person.get_gramps_id(),
                ...
                event_details = [date, event_type, people, descr, citation]
                self.doc.start_row()
        ...
        if event_handles:
            ...
            title = self._("Literatur")
            mark = IndexMark(title, INDEX_TYPE_TOC, 1)        
            self.doc.start_paragraph("PLC-ReportTitle")
            self.doc.write_text(title, mark)
            self.doc.end_paragraph()

#            for sc1, sc2, sc3 in source_set:
#                self.doc.start_paragraph("PLC-Details")
#                self.doc.write_text(sc1),
#                self.doc.write_text(sc2),
#                self.doc.write_text(sc3),
#                self.doc.end_paragraph()

            self.doc.start_table("LitTable", "PLC-LitTable")
            for sc1, sc2, sc3 in sorted(source_set):
                self.doc.start_row()
 #           for detail in event_details:
                self.doc.start_cell("PLC-Cell")
                self.doc.start_paragraph("PLC-Details")
                self.doc.write_text("%s " % sc1)
                self.doc.end_paragraph()
                self.doc.end_cell()
                self.doc.start_cell("PLC-Cell")
                self.doc.start_paragraph("PLC-Details")
                self.doc.write_text("%s " % sc2)
                self.doc.end_paragraph()
                self.doc.end_cell()
                self.doc.start_cell("PLC-Cell")
                self.doc.start_paragraph("PLC-Details")
                self.doc.write_text("%s " % sc3)
                self.doc.end_paragraph()
                self.doc.end_cell()
                self.doc.end_row()
            self.doc.end_table()
...
#
#------------------------------------------------------------------------
class MYPlaceOptions(MenuReportOptions):
